...
blockMode === MoreMultiline && 语言支持 BlockTrimmer
    → 连续接受次数 < consecutiveAcceptThreshold (默认 3)
        → 单行
...
```
连续接受次数 >= consecutiveAcceptThreshold (默认 3) && 前面步骤未命中多行
→ 多行（afterAcceptStrategy）
```

**计数规则**：
- 每次 `hasAcceptedCurrentCompletion()` 返回正数（用户输入完整匹配了当前 ghost text）时计数 +1
- 补全被拒绝（`Rejected` 生命周期）或 `clear()` 时计数归零
- 计数归零后需要重新累积
...
| stopTokens | `['\n\n']` | 遇空行即停 |
| maxTokens | 128 | token 上限 |
| finishedCb | `takeNLines(multilineAfterAcceptLines)` | 取前 N 行截断（默认 1 行） |
...
| stopTokens | `['\n']` | `[]` | `['\n\n']` |
| maxTokens | 64 | 100-150 | 128 |
| finishedCb | 无 | 按模式可选 | takeNLines(1) |
...
| multilineAfterAcceptLines | 1 | 接受后多行策略的截断行数 |
| consecutiveAcceptThreshold | 3 | 连续接受补全多少次后触发多行策略 |
...
| `src/inlineCompletion/cache/currentGhostText.ts` | 连续接受计数追踪 |
