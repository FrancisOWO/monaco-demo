...
6. Debounce 75ms
7. StandardAICompletionClient.requestCompletionStreaming() → 首个 token
...
FullGhostTextController.getCompletions()
  ...
  ├─► Debounce 75ms
  │
  └─► StandardAICompletionClient.requestCompletionStreaming()
        │
        ├─► 首个 token ──► 立即返回
        │
...
│
├── llm/
│   └── StandardAICompletionClient.ts          # 流式 LLM 客户端
│
├── postProcess/
...
| SimpleAICompletionClient | StandardAICompletionClient |
